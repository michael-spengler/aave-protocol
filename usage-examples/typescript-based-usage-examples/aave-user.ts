...
export class AaveUser {
    ...
    private reserveData: any
    private assetConfiguration: any
    ...
    public async borrowDAIFromAave(amountOfDAIToBeBorrowed: number) {
        ...
        const lendingPoolAddress = await AaveWeb3Proxy.aaveProviderContract.methods.getLendingPool().call()
        const lendingPoolContract = new AaveWeb3Proxy.web3Connection.eth.Contract(aaveLendingPoolABI, lendingPoolAddress);

        const userAccountData = await lendingPoolContract.methods.getUserAccountData(process.env.ACCOUNT).call()

        lendingPoolContract.methods.borrow(
